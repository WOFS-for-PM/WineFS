...
extern int winefs_notify_change(struct dentry *dentry, struct iattr *attr);
int winefs_getattr(const struct path *path, struct kstat *stat, u32 request_mask, unsigned int flags);
...
void __winefs_truncate_blocks(struct inode *inode, loff_t start,
				    loff_t end);
