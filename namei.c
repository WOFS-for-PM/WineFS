 ...
 * with d_instantiate().
 */
static int winefs_create(struct inode *dir, struct dentry *dentry, umode_t mode, bool excl)
{
	struct inode *inode = NULL;
...
static int winefs_mknod(struct inode *dir, struct dentry *dentry, umode_t mode, dev_t rdev)
{
	struct inode *inode = NULL;
	int err = PTR_ERR(inode);
	winefs_transaction_t *trans;
...
static int winefs_symlink(struct inode *dir,
...
static int winefs_mkdir(struct inode *dir,
...
static int winefs_rename(struct inode *old_dir,
